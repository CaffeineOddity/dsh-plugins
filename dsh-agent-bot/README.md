...
- 配置站 `GET /agent-bot`：skills / prompts / agents / chat / settings / log。设置页 Client 入口只打开配置页，不配通道 token。
- `agent_ask` 工具：当前会话的 agent 在对话流里调用它转交给 agent-bot 的 agent（设计师等），回复随 tool result 返回（消息 -> 工具调用 -> 结果，无命令卡片）。`/agent_<slug>` 命令保留为主动触发入口。
