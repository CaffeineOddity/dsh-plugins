...
**重试有上限**（`deliverMaxAttempts`，缺省 5）：连败到上限就把文本**写进 md 正文**并标注
「⚠️ 最终答复（未能发到群）」、停止重投 —— 消息不丢，也不至于每轮空转（任务仍留 `running/` 等人处理）。

**收口这一步不设人为窗口**：叫醒用指纹去重（不刷屏），之后**就等它这一轮结束**（`await whenIdle()`），
拿到产出再交付。原来那个"等 W 秒"的窗口纯属多余：超时的后果只是"下次再来"，不构成保护，
却会引入「重试被指纹挡死 ⇒ 永远不交付 / 待决永远清不掉」这类坑。

两个必须配套的细节：
1. **先确认"这一轮真的开始了"再等**：`followup` 之后 agent 可能还没开跑，此时 `whenIdle()` 会立刻 resolve
   → 取到空文本 → 会发出「（综合无输出）」并移 `done/`（假成功）。判据 = `live.status === 'running'`，
   或"已有 `seq > 起点` 的新事件"；都不满足就本轮不动，等下次触发。
2. **产出按 seq 区间取，起点是"叫醒之前"的 seq**：这样晚一点读、重试再读都取到同一段，不会丢。
   取到空文本**不投递**（不发明假答案），留着下次再来。

真正的窗口只有两个，都不能去掉：**入站 `ask` 的 `agent_wait_timeout_ms`**（人在等回复，到点要先回「处理中」）
与**探针判断"会话缺失"的宽限 W**（会话可能正在拉起）。
